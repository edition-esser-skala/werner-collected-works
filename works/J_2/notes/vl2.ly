\version "2.24.0"

J-IIViolinoII = {
  \relative c' {
    \clef treble
    \key c \major \time 3/4 \tempoJ-IIa
    c'16\fE g e c c' g e c c' g e c
    g'' d h g g' d h g g' d h g
    c' g e c c' g e c c' g e c
    h' g d h h' g d h h' g d h
    g' e c g g' e c g g' e c g %5
    a' f c a a' f c a a' f c a
    a' fis d a a' fis d a a' fis d a
    h' g d h h' g d h h' g d h
    c' g e c c' g e c c' g e c
    c' a e c c' a e c c' a e c %10
    a' f d a a' f d a a' f d a
    h' g d h h' g d h h' g d h
    c' g e c g' d h g h' g d h
    c' g e c g' e c g e' c g e
    c4 r8 e\p gis' e %15
    a16 e c a a' e c a a' e c a
    h' gis e h h' gis e h h' gis e h
    c' a e c c' a e c c' a e c
    h' gis e h h' gis e h h' gis e h
    a' e cis a a' e cis a a' e cis a %20
    a' f d a a' f d a a' f d a
    e' h gis e e' h gis e e' h gis e
    e' c a e e' c a e e' c a e
    f' d a f e' h gis e h'' gis e h
    c'\f a e c c' a e c c' a e c %25
    a' f d a a' f d a a' f d a
    h' g d h h' g d h h' g d h
    c g e c e' c g e g' e c g
    c' g e c g' d h g h' g d h
    c' g e c g' e c g e' c g e \noBreak %30
    c2 r4\fermata \bar "||"
    \key c \dorian \time 4/4 \tempoJ-IIb \newSpacingSection
      R1*2
    r2 g'4. g8
    es'4 c r as~ %35
    as g fis2
    g4 d'2 c8 b!
    as2. g8 as
    b4 b,8 c d4 c8 h \noBreak
    c4.-\critnote c8 h2\fermata \bar "||" %40
    \key c \major \time 6/4 \tempoJ-IIc \newSpacingSection
      R1.*4
    r2 r4 r c\fE d %45
    e e f g a h
    c c, f2 e4 fis
    g8 d g f e4 h c d
    e2 a4 d,4. e8 f!4~
    f e d cis4. d8 e4~ %50
    e d c h4. c8 d4~
    d c h8 a g4 g' f8 e
    d g f e d g e4 a8 g e f
    g4 gis a h gis8 a h4~
    h a g8 f e4 a8 g f4 %55
    e2 d4 c4. d8 e4
    f e f g e8 f g4~
    g f e d e8 h' c d
    e4. e8 d4 c d8 c d e
    f2 f4 e g e %60
    f d2 e r4
    r2 r4 r h8 c d4~
    d c h a f! r
    a h2 c4 c8 d e4~
    e d c h e c %65
    c2 h4 c e d
    e d c e d2 \noBreak
    e1.\fermata \bar "||"
    \time 4/4 \tempoJ-IId \newSpacingSection
      r8 \mvTr e16\p-\markup \remarkE "sempre" e, d' e, c' e, h'8 e, r4 \noBreak
    r8 c'16 d e a, f' a, gis e a e h' e, gis h %70
    c a a'8 h, gis' a,16 h c d e h fis' h,
    g'8 e, r h' fis' h,, r h'
    e e,16 fis g8 a h h,16 cis dis8 h
    e a h h, e16 h' g' h, fis' h, e8
    d! h, r fis' cis' fis, r fis' %75
    d h,16 cis d8 e fis gis ais fis
    h h,4 ais8 h4 r8 h''
    e, e,4 c'!8 h fis gis e
    a16 a, e' fis gis e a e h'8 a' g! h,
    c c, h a gis4 r8 h' %80
    e, a d, e a, c'4 c8
    a'16 d, fis d g d a' d, h'8 d, g, a16 h
    a d, fis d g d a' d, h' d, c' d, d'8 g
    fis16 d a d, g d a' d, h' d, c' d, d'8 h'
    a16 d, fis d g d a' d, h' d, fis d g d a' d, %85
    h'8 g a, fis' g16 g,\f h g c g d' g,
    e' g, c e d g, g' d e c g' c, a' f d h'
    c c, e c d g, g' d e c e g d g, g' f
    e c a'8 g g e16 g d g e g h, g'
    c,8 a' d, h' c16 g e c d,8 h' \noBreak %90
    c1\fermata \bar "||"
    \time 3/4 \tempoJ-IIe \newSpacingSection
      R2.*18 %109
    r4 \mvDl h8\fE g h d %110
    g,4 e e'~
    e e d
    cis2 cis4
    cis cis cis
    d f e %115
    d2 d4
    c!2.
    c4 c2
    c4 c c
    cis4. cis8 d d %120
    e4 a, e'
    f2 d4
    d2.
    c!4 h2 \noBreak
    a2.\fermata \bar "||" %125
    \time 4/4 \tempoJ-IIf \newSpacingSection
      g8\pE g g g gis gis gis gis \noBreak
    gis gis gis gis a a a a
    g g g g f f f f
    f f e e f f f f
    g g c c c c b b %130
    b b a a a a a a
    b b b b a a d d
    h h c c c c h h
    h h a a a a g g
    g g g g f f f f %135
    fis fis fis fis fis fis g g
    fis fis fis fis e e d! d \noBreak
    c c c c h2\fermata \bar "||"
    \time 3/4 \tempoJ-IIg \newSpacingSection
      R2.*19 %157
    e'8\f e16 e e8 e c g
    e e' g e h' e,
    c' a e' c a e %160
    fis d a' c, d, c'
    h g d4 fis'
    g8 d h g h' g
    e c c' g d' g,
    e' c h d c g %165
    e c g c d, h'
    c, g e' c g' e
    c'4 r r
    c,8 g e' c g' e
    c'4 r r %170
    r8 c e c g' c,
    a f' a f c' a
    f d a' f d d'
    cis a e' cis d a
    b g f d' e, cis' %175
    d d, f a d a
    h! g d' g, e g
    h, g d' h g' d
    h' g d' g, e g
    h, g h d g h %180
    c g e c g' e
    c a e' c a' e
    f d f a d a
    h g d' f, g d
    e c' d, c' g, h' %185
    c g e c g e \noBreak
    c2 r4\fermata \bar "||"
    \time 4/4 \tempoJ-IIh \newSpacingSection
      e1~\pE \noBreak
    e2 \once \tieDashed fis~
    fis1 %190
    h,2 c!
    a h4 c
    cis d2 cis4
    h1
    c!2 a4 h %195
    a2 h8 d'\f h d
    e, c a'2 h8 a16 g \noBreak
    a2 h\fermata \bar "||"
    \time 3/2 \tempoJ-IIi \newSpacingSection
        R1.*6 %204
    r2 d,\fE d %205
    g2. f4 e f8 g
    a2. g4 fis e
    d e8 f g2. f4
    e d c d8 e f2~
    f4 e d e8 f g4 f %210
    e f8 g a2. g4
    f g8 a h2. a4
    g a8 h c2. h4
    a g g2 fis
    g d d %215
    g2. f!4 e f8 g
    a2. g4 f g8 a
    h2. a4 g a8 h
    c2 c, r
    r h h %220
    e2. d4 c2
    h a1
    h2 d g
    f! e1
    a, r2 %225
    R1.
    r2 e' fis
    g1 a2
    d, g1
    g2 r4 e f! g %230
    a g r f g a
    h a r g a h
    c h a g2 fis4
    g2 r r
    r c, c %235
    f!2. e4 d e8 f
    g2. d4 g f
    e e fis g a g
    r fis g a h a
    r g a h c h %240
    a4. g8 g2 fis
    g r h
    e,4 e fis g a g
    r fis g a h a
    r gis a h c h %245
    a h gis1
    a4 c, d e f e
    r d e f g! f
    r e f g a b
    a2. g4 f g8 a %250
    h!4 a g2 g
    g g1
    g2 g4 g g g
    g g g1
    g r2\fermata \bar "|." %255 finis
  }
}

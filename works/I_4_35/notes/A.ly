\version "2.24.0"

I-IV-XXXVAlto = {
  \relative c' {
    \clef treble
    \key c \major \time 4/4 \tempoI-IV-XXXVa \autoBeamOff
    \mvTr c4.\pE^\solo \tuplet 3/2 8 { e16[ d c] } g'4. \tuplet 3/2 8 { h16[ a g] }
    c8[ h16 a] g8 f e16([ d)] c8 r g'
    a16([ h c h] c8) g f16([ g a g] a8) e
    d16([ e f e] f8) e e d r d
    e4 fis\trill g a\trill %5
    h c\trill d8[ c16 h] a8 g
    d([ g16 a] a4)\trill g r
    R1
    r2 c,4. \tuplet 3/2 8 { e16[ d c] }
    g'4. \tuplet 3/2 8 { h16[ a g] } c8[ h16 a] g8 f %10
    e16([ d)] c8 r g' a h16([ c)] d8 c
    h16([ a)] g8 r d e g f e16 d
    e8 g f16[ e f d] e[ c d e] f[ g a h]
    c8[ h16 a] g8 f e \tuplet 3/2 8 { f16([ e d)] } c8([ d)]\trill
    c4 r r2 %15
    R1
    r8 h' e, h' c16([ h c h)] a4
    g!8 g a16([ g)] f([ e)] f[ f g a] \tuplet 3/2 8 { \sbOn g[ a g f g f]\sbOff }
    e[ e fis g] \tuplet 3/2 8 { \sbOn \hA fis[ g \hA fis e \hA fis e] \sbOff } dis8 h r4
    e fis\trill g a\trill %20
    h8([ a16 g)] fis8 e g4( \hA fis)\trill
    e r r2
    R1
    r2 fis4~ fis16[ a] g([ fis)]
    g4 gis \sbOn \tuplet 3/2 8 { a16[ h a } \hA gis a] \tuplet 3/2 8 { h[ c h } a h] \sbOff %25
    c8 a r a g4. f8
    e16([ d)] c8 g' g a8.[( g16] f[ e)] d([ c)]
    c8\trill h r4 c4. \tuplet 3/2 8 { e16[ d c] }
    g'4. \tuplet 3/2 8 { h16[ a g] } c8[ h16 a] g8 f
    e16([ f e f] g8) a c,8.([ d16] d4)\trill %30
    c r r2
    R1 \noBreak
    R\fermata \bar "||"
    \tempoI-IV-XXXVb r8 d d d d16[ h c d] e[ fis g a] \noBreak
    h[ d, e fis] g[ a h c] d8 g, r h %35
    a16[ fis g a] h[ a g \hA fis] g[ e \hA fis g] a[ g \hA fis e]
    fis[ d e \hA fis] g[ \hA fis e d] e[ d c d] e[ \hA fis e \hA fis]
    g[ d e fis] g[ a h g] c[ h a g] \hA fis[ e d c]
    h8 \tempoI-IV-XXXVc d d e16([ fis)] g8 g16 g g8 a16([ h)]
    cis,2 d4 r %40
    fis8 fis a8. g16 e8 e r h'
    h([ gis)] e16([ d!)] e8 f4 f
    e8 gis h d c([ a)] g?([ e)]
    e h r4 r2\fermata \bar "||"
    \twofourtime \time 2/4 \tempoI-IV-XXXVd
      e4 h \noBreak %45
    h'8 a16([ g)] fis8 e
    fis16([ g a h] a8) g16([ \hA fis)]
    g8 e r4
    R2*3 %51
    g4 d
    d'8 c16([ h)] a8 g
    a16([ h c d] c8) h16([ a)]
    h8 g h a16([ g)] %55
    a4 a8 d,
    g h h a16([ g)]
    fis8. g16 a8 d,
    e g h d,
    d cis r a' %60
    a([ g)] r h
    h([ a)] a([ g)]
    fis16[ e] d8~ d16[ e] \hA fis([ g)]
    a8 g16([ fis)] h8 d,
    fis4( e)\trill %65
    d r
    R2*3
    g4 d %70
    d'8 c16([ h)] a8 g
    a16([ h c d] c8) h16([ a)]
    h8 g h g
    e4 e8 e
    c' c16([ h)] a([ g)] fis([ e)] %75
    dis8. e16 fis8 g16([ a)]
    h2~
    h4. cis16([ dis)]
    e8 e,16([ fis)] g8([ \hA fis)]
    e4 r %80
    R2*5 %85
    e4 r8 e
    e16([ fis)] gis([ a)] h8 e,
    c' a c4~
    c8 h16([ a)] g!8 f!
    f e g4~ %90
    g8[ a16 g] f8 e
    d g d e
    d4 r8 e
    d[ e16 fis] g[ a h c]
    h4 a\trill %95
    g r
    R2
    r8 c, g'4~
    g8 f!16([ g)] a8 c,
    c h r g' %100
    e d r g
    e d g4
    a16([ g f e)] a([ g f e)]
    f8([ g)] a b
    f4( e) %105
    d r
    R2*3
    r4 r8 g %110
    e d r g
    e d g4
    a8[(\trill h16 c]) h8[(\trill c16 d])
    c8[( h16 a] g[ f e d])
    c8 a' d,4\trill %115
    c r
    r8 a' d,4\trill
    c r\fermata \bar "|." %118 finis
  }
}

I-IV-XXXVAltoLyrics = \lyricmode {
  Sal -- _ _ _
  _ ve Re -- gi -- na, Re --
  gi -- na sal -- ve,
  sal -- ve ma -- ter mi --
  se -- ri -- cor -- _ %5
  _ _ _ di -- ae,
  sal -- ve.

  Sal -- _
  _ _ _ ve Re -- %10
  gi -- na, Re -- gi -- na, sal -- ve
  ma -- ter mi -- se -- ri -- cor -- di -- ae,
  sal -- ve, sal -- _ _
  _ ve Re -- gi -- na, __ sal --
  ve. %15

  Vi -- ta, dul -- ce -- do
  et spes no -- stra, sal -- _
  _ _ _ ve,
  et spes no -- stra %20
  sal -- ve, Re -- gi --
  na,

  et __ spes
  no -- stra sal -- _ %25
  _ ve, spes no -- stra
  sal -- ve, et spes no -- stra
  sal -- ve, sal -- _
  _ _ _ ve Re --
  gi -- na, sal -- %30
  ve.

  Ad te cla -- ma -- _ %34
  _ _ _ mus, cla -- %35
  ma -- _ _ _
  _ _ _ _
  _ _ _ _
  mus, cla -- ma -- mus, ex -- u -- les fi -- lii
  E -- vae, %40
  ad te su -- spi -- ra -- mus, ge --
  men -- tes et flen -- tes
  in hac la -- cry -- ma -- rum
  val -- le.
  E -- ia %45
  er -- go, ad -- vo --
  ca -- ta
  no -- stra,

  e -- ia %52
  er -- go, ad -- vo --
  ca -- ta
  no -- stra, il -- los %55
  tu -- os mi --
  se -- ri -- cor -- des
  o -- cu -- los ad
  nos, ad nos con --
  ver -- te, ad %60
  nos, __ ad
  nos __ con --
  ver -- te, __ ad
  nos, ad nos con --
  ver -- %65
  te.

  E -- ia %70
  er -- go, ad -- vo --
  ca -- ta
  no -- stra, il -- los
  tu -- os mi --
  se -- ri -- cor -- des %75
  o -- cu -- los ad
  nos, __
  ad
  nos con -- ver --
  te. %80

  Et, et %86
  Je -- sum, be -- ne --
  di -- ctum fru --
  ctum ven -- tris
  tu -- i, no -- %90
  bis post
  hoc ex -- i -- li --
  um o --
  sten -- _
  _ _ %95
  de,

  ex -- i --
  li -- um o --
  sten -- de. O %100
  cle -- mens, o
  pi -- a, o
  dul -- cis __
  vir -- go Ma --
  ri -- %105
  a,

  o %110
  cle -- mens, o
  pi -- a, o
  dul -- cis
  vir --
  go Ma -- ri -- %115
  a,
  Ma -- ri --
  a. %118 finis
}

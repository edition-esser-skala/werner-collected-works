\version "2.24.0"

B-LXIIIKyrieTenore = {
  \relative c' {
    \clef "treble_8"
    \key f \lydian \time 3/2 \tempoB-LXIIIKyrie \autoBeamOff
    c2. a4 d2
    h4( g) c2 a
    h2.( e4) c a~
    a d h1
    c2 r c~ %5
    c4 a d2 h4 g
    c2 r d~
    d4 h e2 c4 a~
    a d h2. g4
    c2. a2 d4~ %10
    d c h1\trill
    a r2
    r r c
    a4( c) d2 h4( d)
    e2( c) a %15
    r r c~
    c4 a d2 h4( g)
    c2 a d
    g,4 a g1 \noBreak
    a1.\fermata \bar "||" %20
    \time 4/2 \tempoB-LXIIIChriste \newSpacingSection
      \set Staff.timeSignatureFraction = 2/2
      r1 a \noBreak
    c2 c h2. cis4
    d a d1 c4 d
    e2. a,4 h2. a8[ g]
    a1 h2 e %25
    a, d1( c!2)
    b1 a2 r
    a1 c2 c
    h2. cis4 d a d2
    c1 r %30
    a c2 c
    h1 a2 d
    c2._( h8[ a)] g2 r
    r4 g b c d a d e
    f e d2. cis4 d2 %35
    cis\breve\fermata \markKyrieUtSupra \bar "||" %36 finis
  }
}

B-LXIIIKyrieTenoreLyrics = \lyricmode {
  Ky -- ri -- e
  e -- lei -- son,
  e -- lei -- _
  _ _
  son, Ky -- %5
  ri -- e e -- lei --
  son, Ky --
  ri -- e e -- _
  lei -- son, e --
  lei -- son, e -- %10
  _ lei --
  son,
  e --
  lei -- son, e --
  lei -- son, %15
  Ky --
  ri -- e e --
  lei -- son, e --
  lei -- _ _
  son. %20
  Chri --
  ste e -- lei -- _
  _ _ _ _ _
  _ _ _ _
  _ son, Chri -- %25
  ste e --
  lei -- son,
  Chri -- ste e --
  lei -- _ _ _ _
  son, %30
  Chri -- ste e --
  lei -- son, e --
  lei -- son,
  e -- lei -- _ _ _ _ _
  _ _ _ _ _ %35
  son. %36 finis
}

B-LXIIICredoTenore = {
  \relative c' {
    \clef "treble_8"
    \key f \lydian \time 4/2 \tempoB-LXIIICredo \autoBeamOff
      \set Staff.timeSignatureFraction = 2/2
    a1( b)
    a r2 a
    a a a d~
    d4 c h! a g2 g
    a a d d4 d %5
    h1 a
    a g
    g2. g4 a1
    a2. a4 h1
    r2 d d4( h) g2 %10
    g2. g4 g1
    R\breve
    c2 e d c~
    c a d2. c4
    b2. a4 g2 a %15
    d4( c d2) g,1
    r2 a2. a4 a2
    a2. a4 a2 a
    a2. a4 a2 b~
    b b a b %20
    d4( c b2) b b
    g g( c2.) c4
    a2 a( d) b4 b
    c2. c4 c1
    r r2 f, %25
    c' a d c
    f( e4 d) c2 d
    g, a a g
    g c d g,
    g1 g2 g %30
    a c2. h4 a2
    g d' e d
    c1. h!2
    c r r a
    h c2. c4 b2 %35
    a d g,1
    r f2 f'
    e d4 c d2. d4
    c2 c c1
    r2 b d1 %40
    r2 c2. c4 b2
    a2. a4 g1
    r2 h! h h
    h e a,2. a4
    h1 r2 e %45
    e cis a b
    a1 a
    r2 f' b,1~
    b1. b2
    a1 as~ \noBreak %50
    as g\fermata \bar "||"
    \time 3/2 \tempoB-LXIIIEtIncarnatus
      \unset Staff.timeSignatureFraction
      R1. \noBreak
    r2 e c'
    h1 a2
    gis1 a2 %55
    h h h
    h4( a) a2 r
    r a4( h) cis2
    d4( cis d2) e
    e( d) e %60
    d2. d4 c!( d)
    e1( dis2)
    e1 r2
    c1 f,2
    e1 f2 %65
    c'2. c,4 c2
    r h'! c
    d4( c d2) c
    h2. h4 h2
    c c c %70
    c1.~
    c1 h2 \noBreak
    c1 r2\fermata \bar "||"
    \time 4/2 \tempoB-LXIIICrucifixus
      \set Staff.timeSignatureFraction = 2/2
      R\breve \noBreak
    r1 g %75
    as h!
    c2 c2. g4 es'2~
    es d4( c) h! g8[ a] h4 a8[ h]
    c2. h!8[ c] d4 c d2
    g,1 c~ %80
    c2 b4( a!) g1
    r r2 d'
    cis( c1) h!2
    c1( g)
    r c2 c~ %85
    c4 a! b2 a1
    g2 r r1
    r r2 d'(
    es) h! c g
    as g as c %90
    as1. g4( f) \noBreak
    g\breve\fermata \bar "||"
    \tempoB-LXIIIEtResurrexit
      r1 r2 g \noBreak
    g2. g4 g2 g
    r c d e %95
    a, d g, g~
    g fis g a
    fis g4 g g2( fis)
    g1 r
    g h %100
    d e
    c, e
    g a
    h2 c1 h2
    r c( h) d %105
    c1 d2 d
    c( e2.) e4 c2
    h1 a
    a g!2. g4
    f2 b b c %110
    c e a, d
    g,1 a
    a b
    a2 d1 d2
    r d d2. d4 %115
    d2 b c c
    d d g,2. g4
    a2 d1 c4 b
    c1 f2 f
    b,2. b4 d2 g,~ %120
    g c a e
    f2. f4 c2 c'~
    c a d b
    f'( b,) f1
    r2 d'1 h!2 %125
    e2. e4 c2 a
    f c'4 c g1
    g2 a g1
    a2.( b4) c2 cis
    a d cis1 %130
    r b2. b4
    a2 b a1
    g r
    r2 h! h2. h4
    c2 c e( h) %135
    c1 r
    d1. h2
    g c a1
    r g2 g
    g1 g %140
    R\breve
    r2 g c1
    a2 d1( h2)
    g c1 a2~
    a a gis a~ %145
    a gis a1
    a g2. g4
    a2 e' a, d
    b a1 d2~
    d c!1 b2 %150
    c d1 d2
    d2. d4 e2 d~
    d4 cis d1 cis2
    r d1 g,2
    a f c'2 b %155
    c2. d4 e2 f
    r c2.( d4) e2
    a, d2. c4 d2
    g,1 r
    r2 f a2.( h!4) %160
    c2 a b a4( b)
    c2 f, g a
    f4( g) a2 b a4( g)
    f( g) a( h c2 d)
    g, r r1 %165
    r f
    c'2 a f' e4 d
    c2 b4 a g2 c,
    r1 a'2( g4 f)
    e2 g( a g) %170
    c,1 r
    c'2. c4 d2 e
    f e4 d c2 b
    g a g1
    a2 f b a4 g %175
    f2 g f1
    g2 a g1
    a r\fermata \bar "|." %178 finis
  }
}

B-LXIIICredoTenoreLyrics = \lyricmode {
  Pa --
  trem o --
  mni -- po -- ten -- _
  _ _ _ tem, fa --
  cto -- rem coe -- li et %5
  ter -- rae,
  vi -- si --
  bi -- li -- um
  o -- mni -- um
  in -- vi -- si -- %10
  bi -- li -- um.

  Et in u -- num __
  Je -- _ _
  _ _ _ sum %15
  Chri -- stum,
  Fi -- li -- um
  De -- i u -- ni --
  ge -- ni -- tum, et __
  ex Pa -- tre %20
  na -- tum an --
  te o -- mni --
  a, o -- mni -- a
  sae -- cu -- la.
  De -- %25
  um de De -- o,
  De -- o, lu --
  men de lu -- mi --
  ne, de De -- o
  ve -- ro, ge -- %30
  ni -- tum non fa --
  ctum, non fa -- _
  _ _
  ctum, con --
  sub -- stan -- ti -- a -- %35
  lem Pa -- tri,
  per quem
  o -- mni -- a fa -- cta
  sunt, per quem,
  per quem %40
  o -- mni -- a
  fa -- cta sunt.
  Qui pro -- pter
  nos, nos ho -- mi --
  nes pro -- %45
  pter no -- stram sa --
  lu -- tem
  de -- scen --
  dit
  de coe -- %50
  lis.

  In -- car --
  na -- _
  _ tus %55
  est, in -- car --
  na -- tus,
  in -- car --
  na -- tus
  est __ de %60
  Spi -- ri -- tu __
  San --
  cto
  ex Ma --
  ri -- a %65
  Vir -- gi -- ne,
  ex Ma --
  ri -- a
  Vir -- gi -- ne,
  et ho -- mo %70
  fa --
  ctus
  est.

  Cru -- %75
  ci -- fi --
  xus e -- ti -- am __
  pro no -- _ _ _
  _ _ _ _ _
  bis, pro __ %80
  no -- bis
  sub
  Pon -- ti --
  o __
  Pi -- la -- %85
  to, Pi -- la --
  to,
  pas --
  sus et se --
  pul -- tus, et se -- %90
  pul -- tus
  est.
  Et
  re -- sur -- re -- xit
  ter -- ti -- a, %95
  ter -- ti -- a di --
  _ e se --
  cun -- dum Scri -- ptu --
  ras,
  et a -- %100
  scen -- dit,
  et a --
  scen -- dit
  in coe -- lum,
  se -- det, %105
  se -- det ad
  dex -- te -- ram
  Pa -- tris.
  Et i -- te --
  rum ven -- tu -- rus %110
  est cum glo -- ri --
  a iu --
  di -- ca --
  re vi -- vos
  et mor -- tu -- %115
  os, cu -- ius re --
  gni non e -- rit
  fi -- _ _ _
  nis. Et in
  Spi -- ri -- tum San -- %120
  ctum, San -- ctum,
  Do -- mi -- num et __
  vi -- vi -- fi --
  can -- tem,
  qui ex %125
  Pa -- tre Fi -- li --
  o -- que pro -- ce --
  dit. Qui cum
  Pa -- tre et
  Fi -- li -- o %130
  si -- mul
  ad -- o -- ra --
  tur
  et con -- glo --
  ri -- fi -- ca -- %135
  tur:
  qui lo --
  cu -- tus est
  per Pro --
  phe -- tas. %140

  Et u --
  nam san --
  ctam, san -- ctam __
  Ec -- cle -- _ %145
  si -- am.
  Con -- fi -- te --
  or u -- num ba --
  ptis -- ma in __
  re -- mis -- %150
  si -- o -- nem
  pec -- ca -- to -- _
  _ _ rum.
  Et ex --
  pe -- cto re -- sur -- %155
  re -- cti -- o -- nem
  mor -- tu --
  o -- _ _ _
  rum,
  et vi -- %160
  tam ven -- tu -- ri __
  sae -- cu -- li, ven --
  tu -- ri sae -- cu --
  li, __ a --
  men, %165
  a --
  _ _ _ _ _
  _ _ _ _ men,
  a --
  men, a -- %170
  men,
  a -- men, a -- _
  _ _ _ _ men,
  a -- men, a --
  men, a -- _ _ _ %175
  _ _ men,
  a -- men, a --
  men. %178 finis
}

B-LXIIISanctusTenore = {
  \relative c' {
    \clef "treble_8"
    \key f \lydian \time 4/2 \tempoB-LXIIISanctus \autoBeamOff
      \set Staff.timeSignatureFraction = 2/2
    r1 c~
    c4 h g2 a2. h4
    c1 r2 c~
    c4 a d2 g, r
    a( g) g c~ %5
    c4 b a b c2 g
    f4( d' c b a g) f2
    g c c1
    c r2 c~
    c4 c h!2 c1~ %10
    c2 h4( a) g1~
    g2 g g1
    r2 a a a
    a1 a2 b
    a1 g %15
    r c(
    d2) b g c
    a1 r
    c2 c1 d2
    g, f4( e) a1( \noBreak %20
    g) a\fermata \bar "||"
    \tempoB-LXIIIOsanna c f, \noBreak
    g\trill f
    r4 a2 h4 c d e d8[ e]
    f4 d a h c g( c) b %25
    a f a2. b4 c a
    f b f g a c2 b4
    a2 f r1
    r c'
    f, g2 g %30
    f d' g,1
    b1 f
    g f
    r c'
    c2( f1) e2 %35
    r4 a,2 h!4 c d e d8[ e]
    f4. e8 d1 c2~
    c b1 a4 g
    a2 f r1
    R\breve %40
    f'1 c
    d c2 c4 f
    e c r c c4. c8 b4 a
    g2 c c2. b8([ a)]
    g2( c) c1\fermata \bar "|." %45 finis
  }
}

B-LXIIISanctusTenoreLyrics = \lyricmode {
  San --
  _ _ _ _
  ctus, san --
  _ _ ctus,
  san -- ctus, san -- %5
  _ _ _ _ ctus,
  san -- ctus,
  san -- ctus, san --
  ctus Do --
  mi -- nus De -- %10
  us __ Sa --
  ba -- oth.
  Ple -- ni sunt
  coe -- li et
  ter -- ra %15
  glo --
  ri -- a tu --
  a,
  o -- san -- na
  in ex -- cel -- %20
  sis.
  O -- san --
  _ na
  in ex -- cel -- _ _ _
  _ _ _ _ sis, in __ ex -- %28
  cel -- _ _ _ _ _
  _ _ _ _ sis, in ex --
  cel -- sis,
  o --
  san -- na in %30
  ex -- cel -- sis,
  o -- san --
  _ na,
  o --
  san -- na %35
  in ex -- cel -- _ _ _
  _ _ _ _
  _ _ _
  _ sis,
  %40
  o -- san --
  _ na in ex --
  cel -- sis, o -- san -- na in ex --
  cel -- sis, in ex --
  cel -- sis. %45 finis
}

B-LXIIIBenedictusTenore = {
  \relative c' {
    \clef "treble_8"
    \key f \lydian \time 3/2 \tempoB-LXIIIBenedictus \autoBeamOff
    d2._( a4 f2)
    cis d1
    a'2. g4 f( e)
    d2 g1
    f2. g4 a2 %5
    d4 d b2 c
    d4 e f2 e
    a, d2. d4
    c2 f, g
    e4 c g'2. g4 %10
    c,1.\fermata \markOsannaUtSupra \bar "||" %11 finis
  }
}

B-LXIIIBenedictusTenoreLyrics = \lyricmode {
  Be --
  ne -- di --
  ctus, qui ve --
  nit, ve --
  nit in no -- %5
  mi -- ne Do -- mi --
  ni, in no -- mi --
  ne Do -- mi --
  ni, in no --
  mi -- ne Do -- mi -- %10
  ni. %11 finis
}

B-LXIIIAgnusTenore = {
  \relative c' {
    \clef "treble_8"
    \key f \lydian \time 4/2 \tempoB-LXIIIAgnus \autoBeamOff
      \set Staff.timeSignatureFraction = 2/2
    a1. d2~
    d h! h e~
    e c d d
    c2. a4 f2( b)
    e,1 r2 a~ %5
    a a b1
    a2 d1 d2
    e1. d4( cis)
    d1 cis
    r\fermata c %10
    c c
    f,2 d' c2.( d4)
    e2 e d es
    a,( \once \stemUp b a1)
    g r2 b %15
    d1 g,2 c
    c1.( h!2)
    c1 r2 c~
    c c d1
    c2 c b( d) %20
    e e, f2.( g4)
    a2 d c a \noBreak
    b1 c\fermata \bar "||"
    \time 3/2 \tempoB-LXIIIDona \newSpacingSection
      \unset Staff.timeSignatureFraction
      R1. \noBreak
    c2. h4 c2 %25
    a g f4 a~
    a8[ h] c2 h8[ a] g2
    r r r4 a(
    \once \stemUp b a8[ b] g4) c a f
    r g2( c) a4 %30
    r2 r4 d g, g(
    a) g g2 g
    c2. h4 c2
    g a4 b c2~
    c4 a g2( f) %35
    g r r
    r4 c( d c8[ d] b4) c
    a f r2 r
    r4 f( d' c2 b4)
    a2 b a %40
    g a4 d2 c8[ d]
    g,2 a b
    r c( g')
    c, r r
    r4 a b a8[ b] c4 b8[ c] %45
    d4 b g1
    a2 r4 d( c2)
    c r r\fermata \bar "|." %48 finis
  }
}

B-LXIIIAgnusTenoreLyrics = \lyricmode {
  A -- gnus __
  De -- i, qui __
  tol -- lis pec --
  ca -- ta mun --
  di: Mi -- %5
  se -- re --
  re, mi -- se --
  re -- re __
  no -- bis.
  A -- %10
  gnus De --
  i, qui tol --
  lis pec -- ca -- ta
  mun --
  di: Mi -- %15
  se -- re -- re
  no --
  bis. A --
  gnus De --
  i, qui tol -- %20
  lis pec -- ca --
  ta, pec -- ca -- ta
  mun -- di:

  Do -- na no -- %25
  bis pa -- cem, pa --
  _ _ cem,
  pa --
  cem, pa -- cem,
  pa -- cem, %30
  pa -- cem, no --
  bis pa -- cem,
  do -- na no --
  bis, do -- _ _
  na no -- %35
  bis

  pa -- cem,
  pa -- cem,
  pa -- %40
  cem, pa -- cem,
  pa -- _ _ _
  cem, no -- bis
  pa -- cem,
  pa -- _ _ _ _ %45
  _ cem, pa --
  cem, pa --
  cem. %48 finis
}

B-LXIIIFragmentTenore = {
  \relative c' {
    \clef "treble_8"
    \key f \lydian \time 4/2 \tempoB-LXIIIFragment \autoBeamOff
      \set Staff.timeSignatureFraction = 2/2
    r1 c
    e2 d4 c h2 c
    d4 c d2 e2. d4
    c h a2 g1
    r c %5
    c2 c a d~
    d4 c b2. a4 g a8[  h]
    c2. b4 a2. g4
    f1 s %9 finis
  }
}

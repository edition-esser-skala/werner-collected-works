\version "2.24.0"

B-VIKyrieBasso = {
  \relative c {
    \clef bass
    \key d \major \time 4/4 \tempoB-VIKyrie \autoBeamOff
    \mvTr d4.\fE^\tutti d8 g4 h
    e, a d,4.( cis8)
    d2 a4 r
    f'4. f8 g4 \hA f
    e f8 dis e2 %5
    a,8 r r4 dis8. dis16 dis8 dis
    e e r4 r8 e dis dis
    r d cis cis r d h4
    a8 g' fis([ cis)] d([ cis)] h([ a)] \noBreak
    gis2 a\fermata \bar "||" %10
    \tempoB-VIKyrieB d8([ e)] fis([ g)] fis([ g)] fis([ e]) \noBreak
    d([ h' a g] fis[ g fis e)]
    d4 h'8([ a)] g([ fis16 e] a8[ g)]
    fis([ e)] d([ fis)] cis([ h)] a4
    r2 a'4. g8 %15
    fis([ e)] d([ cis)] d([ cis16 h] e8[ d]
    cis[ h)] a4 r8 h' a([ g]
    fis[ g fis e)] d h' a([ g]
    fis[ g fis e)] d4 r
    r2 a' %20
    a a
    a a(
    h) a8([ g)] fis([ e)]
    d([ cis)] h a' g([ fis]) e([ d)]
    cis h a g' fis([ g a a,)] %25
    d4 r r2
    R1*3
    r4 r8 \mvTr ais'\pE^\soloE h \tuplet 3/2 8 { fis16([\trill e fis)] } g8[ \tuplet 3/2 8 { gis16\trill fis \hA gis] } %30
    a8[ \tuplet 3/2 8 { e16\trill d e] } fis8[ \tuplet 3/2 8 { fis16\trill e fis] } g4 r8 e
    fis h fis4 h, r
    R1*8 %40
    \mvTr d8([\fE^\tutti e)] fis([ g)] fis([ g)] fis([ e)]
    d([ h' a g] fis[ g fis e)]
    d4 h'8([ a)] g([ fis16 e] a8[ g])
    fis e fis([ e16 d] cis8[ h)] a4
    r8 d cis([ h)] a a'16([ g)] fis8([ e)] %45
    d([ c)] h([ a)] g([ g' h, g)]
    d'4 r8 d g([ g, h g)]
    d'4 r8 dis e([ h' g e)]
    h4 r8 e c'([ a h h,)]
    e4 r r8 e d([ c] %50
    h[ c h a)] g e' d([ \hA c]
    h[ c h a)] g4 e'8([ d)]
    c[( h16 a] d8[ \hA c] h) g c'([ h)]
    a[ g16 fis] h8[ a] g[ fis16 e] a8[ g]
    fis[ g fis e] d4 r8 g %55
    fis([ g fis e)] d4 r8 g
    fis([ g fis e)] d4 r8 a'
    g[ fis16 e] a8[ g] fis[ e16 d] g8[ fis]
    e[ d16 cis] fis8[ e] d[ cis16 h] e8[ d]
    cis[ h16 a] d8[ h] g'[ fis16 e] a8[ g] %60
    fis d r4 r2
    d8 d' cis h a([ g fis e])
    d([ cis)] h([ a)] g([ a h cis)]
    d([ e)] fis([ d)] e([ h cis a)]
    d([ e)] fis([ g)] a4 fis8 h %65
    g4( a) d,8 d cis([ d)]
    a' d, cis([ d)] a a' fis([ d]
    a'[ g a a,)] d4 r
    R1\fermata \bar "|." %69 finis
  }
}

B-VIKyrieBassoLyrics = \lyricmode {
  Ky -- ri -- e e --
  lei -- son, e --
  lei -- son,
  Ky -- ri -- e e --
  lei -- son, e -- lei -- %5
  son, Ky -- ri -- e e --
  lei -- son, e -- lei -- son,
  e -- lei -- son, e -- lei --
  son, e -- lei -- son, e --
  lei -- son. %10
  Ky -- ri -- e __ e --
  lei --
  son, e -- lei --
  son, e -- lei -- son,
  Ky -- ri -- %15
  e __ e -- lei --
  son, e -- lei --
  son, e -- lei --
  son,
  Ky -- %20
  ri -- e
  e -- lei --
  son, e --
  lei -- son, e -- lei -- son,
  Ky -- ri -- e e -- lei -- %25
  son.

  Chri -- ste e -- lei -- %30
  _ _ son, Chri --
  ste e -- lei -- son.

  Ky -- ri -- e __ e -- %41
  lei --
  son, e -- lei --
  son, e -- lei -- son,
  e -- lei -- son, e -- lei -- %45
  son, e -- lei --
  son, e -- lei --
  son, e -- lei --
  son, e -- lei --
  son, e -- lei -- %50
  son, e -- lei --
  son, e --
  lei -- son, e --
  lei -- _ _ _
  _ son, e -- %55
  lei -- son, e --
  lei -- son, e --
  lei -- _ _ _
  _ _ _ _
  _ _ _ _ %60
  _ son,
  Ky -- ri -- e e -- lei --
  son, e -- lei --
  son, e -- lei --
  son, e -- lei -- son, e -- %65
  lei -- son, e -- lei --
  son, e -- lei -- son, e -- lei --
  son. %68 finis
}

B-VIGloriaBasso = {
  \relative c {
    \clef bass
    \key d \major \time 4/4 \autoBeamOff \tempoB-VIGloria
    \mvTr d4\fE^\tutti r d'4. a8
    fis4 d r2
    dis4 r r \hA dis
    e8. e16 e4 r2
    ais,4 r \hA ais ais %5
    h8. h16 h4 r2
    R1
    r8 h'4 h,8 e4 a
    e2 h4^\critnote a'8 a
    g e cis4 h8 h' g4 %10
    fis8 fis g eis fis2 \noBreak
    h,4 r r2\fermata \bar "||"
    \key g \major \tempoB-VILaudamus
      \tag #'parts { R1*11 \noBreak R1\fermata }
      \tag #'full-score { \B-VILaudamusOrganoSolo } \bar "||"
    \time 3/2 \tempoB-VIGratias \newSpacingSection
      \mvTr e2.\fE^\tutti fis4 g2 \noBreak %25
    fis e dis
    e d4( cis) h2
    e4 e fis1
    e4( fis) g( fis) e2
    a h2. a4 %30
    gis2 a2.( g4)
    fis2 g4( fis) e( d)
    c2 d1
    g,2 r r
    R1.*2 %36
    r2 e'1
    fis2 dis1
    e2 cis d~
    d4 d h2 c %40
    a d2. d4
    g,2 g' a
    fis( g) e
    fis2. fis4 d2
    h cis ais %45
    h1 e2(
    a) fis dis
    e c2. c4
    h2. cis4 dis2
    e h1 \noBreak %50
    e,1.\fermata \bar "||"
    \time 4/4 \tempoB-VIDomine \newSpacingSection
      R1*11 %62
    r2^\aDueE \mvTr fis'8([\pE^\soloE a)] g fis
    g16([ fis)] e8 r4 r e8 a
    g d g16([ h)] a([ g)] fis4 r8 fis( %65
    g16[ fis g8)] fis gis( a16[ \hA gis a8)] g?16([ fis g8)]
    fis16([ h)] g([ e)] h4 e r
    R1*8 \noBreak %75
    R1\fermata \bar "||"
    \key g \minor \tempoB-VIQuiTollis
      \mvTr g2\fE^\tutti g8([ fis)] fis4 \noBreak
    r8 f f f b4 b,
    r8 as'4 as8 g4 \hA as
    r8 fis4 fis8 g4 c, %80
    f( b, es2)
    d4 r r2
    R1*3 %85
    \key d \major \tempoB-VIQuiSedes
      r4 r8 \mvTr d\fE^\tutti cis cis r cis \noBreak
    d d r d g g16 g e8 e
    cis4( d) a2
    R1
    r2 a'8. a16 a8 d, %90
    a2 d8 d4\p fis8
    h, h h d g4 e \noBreak
    cis( d) a2\fermata \bar "||"
    \tempoB-VIQuoniam R1*12 \noBreak %105
    R1\fermata \bar "||"
    \tempoB-VICumSancto \mvTr h'2\fE^\tutti e,4 a \noBreak
    d, cis d4. d8
    \time 6/4 \tempoB-VICumSanctoB \newSpacingSection
      a2 r4 a' a a8 a \noBreak
    fis4 d h' g e a %110
    fis8[ g fis e d fis] a[ g fis e d fis]
    h[ a gis fis \hA gis e] a4. g8[ fis e]
    d4 h'8([ a g fis)] e4 a( a,)
    d2 r4 r2 r4
    r2 r4 a' a a8 a %115
    fis4 d h' g e a
    fis8[ g fis e d fis] g[ fis e d cis h]
    a4 fis'8[ e fis d] g4. fis8[ e g]
    a4. g8[ fis a] h4. a8[ gis fis]
    e[ d cis h a a'] fis([ d)] e2 %120
    a,4 r r r2 r4
    r2 r4 a' a a8 a
    fis4 d h' g e cis
    a'8[ g fis g fis e] d[ e d c h a]
    g[h  e d c h] a[ a' fis e d fis] %125
    g[ a h a h g] d'[ c h a g fis]
    e[ d cis! h a cis] d2 r4
    R1.
    a'4 a a8 a fis4 d h'
    g e2 a8[ g fis g fis e] %130
    d4. e16[ fis] g8[ fis] e4. fis16[ g] a8[ g]
    fis4. g16[ a] h8[ a] g[ fis16 g] a8[ g a a,]
    d4 r8 d'([ cis h] a[ g] fis4) cis
    d8([ e] fis[ g16 a] h4) g a2
    d,4 r r d8 fis a4( a,) %135
    d r r r2 r4\fermata \bar "|." %136 finis
  }
}

B-VIGloriaBassoLyrics = \lyricmode {
  Et, et in
  ter -- ra
  pax ho --
  mi -- ni -- bus,
  pax, pax ho -- %5
  mi -- ni -- bus

  bo -- nae vo -- lun --
  ta -- tis, vo -- lun --
  ta -- tis, bo -- nae vo -- lun -- %10
  ta -- tis, vo -- lun -- ta --
  tis.

  \tag #'full-score { \repeat unfold 71 \skip8 }

  Gra -- ti -- as %25
  a -- gi -- mus
  ti -- bi, __ a --
  gi -- mus ti --
  bi, __ a -- gi --
  mus, a -- gi -- %30
  mus ti --
  bi, a -- gi --
  mus ti --
  bi

  pro -- %37
  pter ma --
  gnam glo -- _
  ri -- am, ma -- %40
  gnam glo -- ri --
  am, pro -- pter
  ma -- gnam
  glo -- ri -- am,
  pro -- pter ma -- %45
  gnam, ma --
  gnam, ma --
  gnam glo -- ri --
  am __ _ _
  _ tu -- %50
  am.

  Do -- mi -- ne %63
  Fi -- li, Fi -- li
  u -- ni -- ge -- ni -- te, Je -- %65
  su, Je -- su, __
  Je -- su Chri -- ste.

  Qui tol -- lis %77
  pec -- ca -- ta mun -- di:
  Mi -- se -- re -- re,
  mi -- se -- re -- re, %80
  no --
  bis.

  Qui se -- des, qui %86
  se -- des ad dex -- te -- ram Pa -- tris,
  Pa -- tris:

  Mi -- se -- re -- re %90
  no -- bis, mi -- se --
  re -- re, mi -- se -- re -- re
  no -- bis.

  Cum San -- cto, %107
  San -- cto Spi -- ri --
  tu in glo -- ri -- a
  De -- i Pa -- tris, a -- men, %110
  a -- _
  _ _ _
  men, a -- men, a --
  men,
  in glo -- ri -- a %115
  De -- i Pa -- tris, a -- men,
  a -- _
  men, a -- _ _
  _ _ _ _
  _ men, a -- %120
  men,
  in glo -- ri -- a
  De -- i Pa -- tris, a -- men,
  a -- _
  _ _ %125
  _ _
  _ men,

  in glo -- ri -- a De -- i Pa --
  tris, a -- _ %130
  _ _ _ _ _ _
  _ _ _ _ _
  men, a -- men,
  a -- men, a --
  men, a -- men, a -- %135
  men. %136 finis
}

B-VICredoBasso = {
  \relative c {
    \clef bass
    \key d \major \time 3/4 \autoBeamOff \tempoB-VICredo
    \mvTr d4\fE^\tutti d8 d cis d
    a'4 a8 d cis d
    a4 a8 fis cis([ d)]
    a4 fis' d
    g8 g e4 a8. a16 %5
    fis4 h8 gis a([ fis)]
    cis([ d)] e4. e8
    a,4 r8 \mvTr a'\pE^\solo fis4
    g!8 g e( a4) fis8
    d fis([ g)] e fis4 %10
    h,8 r r4 r
    r r d8 g
    fis h e,([ fis)] g4
    a8 h c8. c16 h8 d16([ \hA c]
    h[ a g fis] e8.) e16 d4 %15
    R2.*4
    a'8. a16 fis8 d a'16[ h cis a] %20
    d[ h, d e] fis[ g a fis] h[ g, h cis]
    d[ e fis d] g8[ a16 h] a8 g
    fis d d' h gis a
    h cis16([ a)] h8 e, e a
    fis16[ d8 h16] gis'[ e8 cis16] a'[ fis8 d16] %25
    h'[ gis8 e16] cis'[ a] fis d e8. e16
    a,8 \mvTr a'\fE^\tuttiE fis h g! e
    cis d! a fis' dis e
    c' a16 a h8([ h,)] e4
    r r r8 e %30
    cis!4 d8 d a4 \noBreak
    d2 r4\fermata \bar "||"
    \key d \minor \time 4/4 \tempoB-VIEtIncarnatus \newSpacingSection
      R1*8 %40
    r2 \mvTr a'4.\pE^\soloE^\aTre a8
    d, d g g c,4 f
    d2 a4 r
    r8 g' g g c4 f,
    c4.( h8) c2 %45
    r f8 f es es
    d4 g a b!8 g
    a4 a, d d \noBreak
    gis,4. gis8 a2\fermata \bar "||"
    \key d \major \tempoB-VIEtResurrexit
      \mvTr d2\fE^\tutti e4. e8 \noBreak %50
    fis4 fis g2
    a4 a h2
    a4 a fis( h)
    g e a( fis)
    d h e( cis) %55
    fis d e2
    a,4 a'2 g!8([ fis)]
    e([ fis g a] h4 a8[ g)]
    fis4 g a( g8[ fis)]
    e4( fis) g( fis8[ e)] %60
    d4 e fis h8 h
    fis2 h,4 r
    g'2 d4. d8
    a'4 a e4. e8
    h'4 g d'4.( cis8 %65
    h4) h a a
    fis2 d
    g e
    a fis
    h4 g e fis %70
    h, h' g a
    fis d h cis
    d h e a,
    d8([ c)] h([ a)] g4 g'8([ fis)]
    e([ d)] cis!([ h)] a4 d %75
    cis d a2 \noBreak
    d r\fermata \bar "||"
    \time 3/4 \tempoB-VIEtInSpiritum \newSpacingSection
      R2.*18 %95
    r4 \mvTrh \mvDl d'\pE^\soloE^\aQuattro d,
    g g e
    h'4. h,8 h4
    \mvTr g'4.\fE^\tutti d8 h d
    g,4 g h %100
    e4. h'8 g h
    e,4 e a
    d, g4. g8
    d4 d'( h)
    g d2 %105
    g,4 r r
    R2.*6 %112
    r4 r \mvTr a'\pE^\soloE^\aDue
    d,8([ e)] fis([ g)] a([ d,)]
    e4( cis) a %115
    r e'2
    e8([ d)] d4 fis
    gis gis a
    a8([ d)] cis([ h)] a4~
    a8 h a4( gis) %120
    a r r
    R2.*5 \noBreak %126
    R2.\fermata \bar "||"
    \time 4/4 \tempoB-VIEtVitam \newSpacingSection
      R1*9 %136
    \mvTr a2\fE^\tuttiE a
    a a
    fis4 a h4. h8
    a4 r a,8[ h cis d] %140
    e[ fis g a] h,[ cis d e]
    fis fis([ d g] fis[ e] fis4)
    h,8 h'[ a! g] fis[ e d cis]
    h'[ a! g fis] e[ d cis h]
    a'[ g! fis e] d[ c h a] %145
    g[ g'] e([ c)] d([ \hA c] \once \stemUp d4)
    g, r r2
    R1*2
    g'16[( a h8] a[ g] fis[ e d c]) %150
    h g'([ fis e] d[ c h a)]
    g4 r r2
    a' a
    a a
    fis4 g8([ fis)] g([ d)] h([ g)] %155
    d'([ e)] fis[ g] a[ a, cis a]
    h[ cis d e] fis[ fis, a fis]
    g[ a h cis] d[ e fis g]
    a[ h cis a] d[ h] fis([ g)]
    a([ g a a,)] d d'[ cis h] %160
    a[ g fis e] d[ cis h fis']
    g[ fis16 e] a8[ a,] d4 r\fermata \bar "|." %162 finis
  }
}

B-VICredoBassoLyrics = \lyricmode {
  Pa -- trem o -- mni -- po --
  ten -- tem, fa -- cto -- rem
  coe -- li et ter --
  rae, vi -- si --
  bi -- li -- um o -- mni -- %5
  um et in -- vi --
  si -- bi -- li --
  um. Et in
  u -- num Do -- mi --
  num Je -- sum Chri -- %10
  stum,
  et ex
  Pa -- tre na -- tum
  an -- te o -- mni -- a sae --
  cu -- la, %15

  Ge -- ni -- tum non fa -- %20
  _ _ _
  _ _ ctum, non
  fa -- ctum con -- sub -- stan -- ti --
  a -- lem Pa -- tri, per quem
  o -- _ _ %25
  _ _ mni -- a fa -- cta
  sunt. Qui pro -- pter nos, nos
  ho -- mi -- nes et pro -- pter
  no -- stram sa -- lu -- tem
  de -- %30
  scen -- dit de coe --
  lis.

  Cru -- ci -- %41
  fi -- xus e -- ti -- am pro
  no -- bis,
  sub Pon -- ti -- o Pi --
  la -- to %45
  et se -- pul -- tus
  est, se -- pul -- tus, se --
  pul -- tus est, se --
  pul -- tus est.
  Et re -- sur -- %50
  re -- xit ter --
  ti -- a di --
  e se -- cun --
  dum Scri -- ptu --
  ras, se -- cun -- %55
  dum Scri -- ptu --
  ras, et a --
  scen --
  dit in coe --
  lum, __ se -- %60
  det ad dex -- te -- ram
  Pa -- tris.
  Et i -- te --
  rum ven -- tu -- rus
  est cum glo -- %65
  ri -- a iu --
  di -- ca --
  re vi --
  vos, vi --
  vos et mor -- tu -- %70
  os, cu -- ius re --
  gni non e -- rit,
  cu -- ius re -- gni
  non e -- rit, non
  e -- rit, e -- rit, %75
  e -- rit fi --
  nis.

  Qui cum %96
  Pa -- tre et
  Fi -- li -- o
  si -- mul ad -- o --
  ra -- tur et %100
  con -- glo -- ri -- fi --
  ca -- tur: qui
  lo -- cu -- tus
  est per __
  Pro -- phe -- %105
  tas.

  Con -- %113
  fi -- te -- or
  u -- num %115
  ba --
  ptis -- ma in
  re -- mis -- si --
  o -- nem pec --
  ca -- to -- %120
  rum.

  Et vi -- %137
  tam ven --
  tu -- ri sae -- cu --
  li, a -- %140
  _ _
  men, a --
  men, a -- _
  _ _
  _ _ %145
  _ men, a --
  men,

  a -- %150
  men, a --
  men,
  et vi --
  tam ven --
  tu -- ri __ sae -- cu -- %155
  li, __ a -- _
  _ _
  _ _
  _ _ men,
  a -- men, a -- %160
  _ _
  _ _ men. %162 finis
}

B-VISanctusBasso = {
  \relative c {
    \clef bass
    \key d \major \time 4/4 \autoBeamOff \tempoB-VISanctus
    \mvTr d1~\fE^\tuttiE
    d4 h a2
    e'4 r h2
    a cis!
    h r8 a' a a %5
    g4.( fis8 e4) d
    cis8([ h16 a)] h8([ cis)] d4 r
    r8 gis gis gis a4 fis8([ d)] \noBreak
    a4. a8 d2\fermata \bar "||"
    \key g \major \time 3/4 \tempoB-VIPleni \newSpacingSection
      R2.*34 \noBreak %43
    R2.\fermata \bar "||"
    \key d \major \time 4/4 \tempoB-VIOsanna \newSpacingSection
      \mvTr d2\fE^\tuttiE h'4 dis, \noBreak %45
    e8([ fis)] g4 a( cis,)
    d cis^\critnote d8([ e)] fis4~
    fis h, cis8([ d)] e4~
    e a, a' g!8([ a)]
    fis4 d r h %50
    g' e r a,
    fis' d r2
    R1*2
    d2 h'4 dis, %55
    e8([ fis)] g4 a( cis,)
    d fis g e
    fis h fis2
    h,4 h'8([ a!)] g4 e
    r a8([ g)] fis4 d %60
    g4. fis8 e4( d)
    a a'8([ g)] fis4( d)
    g g2 e4
    a2.( fis4)
    h h, cis2 %65
    d4 fis g e
    cis d a2
    d r4 g
    fis8([ g)] a4 r g
    fis8([ g)] a4 h fis^\critnote %70
    g1
    d\fermata \bar "|." %72 finis
  }
}

B-VISanctusBassoLyrics = \lyricmode {
  San --
  ctus, san --
  ctus, san --
  ctus, san --
  ctus Do -- mi -- nus %5
  De -- us
  Sa -- ba -- oth,
  Do -- mi -- nus De -- us
  Sa -- ba -- oth.

  O -- san -- na %45
  in __ ex -- cel --
  sis, in ex -- cel --
  sis, o -- san --
  na in ex --
  cel -- sis, o -- %50
  san -- na, o --
  san -- na,

  o -- san -- na %55
  in __ ex -- cel --
  sis, o -- san -- na
  in ex -- cel --
  sis, o -- san -- na,
  o -- san -- na %60
  in ex -- cel --
  sis, o -- san --
  na in ex --
  cel --
  sis, o -- san -- %65
  na, o -- san -- na
  in ex -- cel --
  sis, o --
  san -- na, o --
  san -- na in ex -- %70
  cel --
  sis. %72 finis
}

B-VIAgnusBasso = {
  \relative c {
    \clef bass
    \key d \major \time 4/4 \autoBeamOff \tempoB-VIAgnus
    R1*9 %9
    \mvTr g'4.\pE^\solo h8 fis([ a)] c fis, %10
    g4 h8 d gis,4. gis8
    a a, c'4. a8 fis d
    h' g, h'4. g8 e c
    a' fis, a'4. fis8 dis h
    g'4 gis a ais %15
    h cis d8([ h)] g([ e)]
    fis2 h,4 r
    \mvTr d4.\fE^\tutti a'8 d([ a)] fis a
    d, d r d g g r e
    cis cis d4 a r %20
    r8 d c a h[( h'16 a)] g8 f \noBreak
    e2 a,4 r\fermata \bar "||"
    \tempoB-VIDona \mvTr d4.\fE^\tutti h'8 a4 cis, \noBreak
    d8[ e16 fis] g[ a h8] a4 cis,
    d8 fis16([ e fis8)] d e4 e, %25
    r8 a'[( fis)] d e4( e,)
    a8 a'4 cis,8 d( g!4) fis8
    e a4 g8 fis[ h e, a]
    d,[ g cis, fis] h, g'([ e fis)]
    h,4 r r2 %30
    r4 h'16([ a! g fis)] e4 a16([ g fis e)]
    d8 fis([ cis d)] a a'16([ g] fis[ e d8]
    cis8) a r d a a'4 g8
    fis( g4) fis8 e( fis4 e8)
    d d'([ cis d)] a4 r8 ais %35
    h4 r8 gis a4 r8 fis
    g4 r8 e fis g c([ a)]
    fis([ g)] d4 g, r
    R1*2 %40
    d'4. h'8 a4 cis,!
    d8[ e16 fis] g[ a h8] a4 cis,
    d8 d4 cis8 h( e4) d8
    cis( fis4 e8) d g!4 fis8
    e( a4) g8 fis([ cis] d4) %45
    a8 a'4 g8 fis( g4 fis8)
    e8 fis4 e8 d g a4
    d,8 r r4 r2
    cis8 d a4 d8 r a' r
    d, r r4 r2\fermata \bar "|." %50 finis
  }
}

B-VIAgnusBassoLyrics = \lyricmode {
  A -- gnus De -- i, qui %10
  tol -- lis pec -- ca -- ta
  mun -- di: Mi -- se -- re -- re
  no -- bis, mi -- se -- re -- re
  no -- bis, mi -- se -- re -- re,
  mi -- se -- re -- re, %15
  mi -- se -- re -- re __
  no -- bis.
  A -- gnus De -- i, qui
  tol -- lis, qui tol -- lis pec --
  ca -- ta mun -- di, %20
  pec -- ca -- ta, pec -- ca -- ta
  mun -- di:
  Do -- na no -- bis
  pa -- _ _ _
  cem, no -- bis pa -- cem, %25
  no -- bis pa --
  cem, do -- na no -- bis
  pa -- _ _ _
  _ cem, pa --
  cem, %30
  do -- na no --
  bis pa -- cem, pa --
  cem, pa -- cem, do -- na
  no -- bis pa --
  cem, pa -- cem, pa -- %35
  _ _ _ _
  _ _ _ cem, no --
  bis __ pa -- cem,

  do -- na no -- bis %41
  pa -- _ _ _
  cem, do -- na no -- bis
  pa -- cem, do -- na
  no -- bis pa -- %45
  cem, do -- na no --
  bis pa -- cem, no -- bis pa --
  cem,
  pa -- cem, pa -- cem, pa --
  cem. %50 finis
}
